...
## System Paths ##
path                 = './'
parameters           = {'0': 'test.txt'} # Dictionary can be expanded for multiple test parameters

## Run Parameters ##
TEST_NUM_SET         = [0] # List can be expanded in case of multiple test parameters
best_model_path_list = ['./1_best_model', './2_best_model', './3_best_model', './4_best_model', './5_best_model', './6_best_model', './7_best_model', './8_best_model', './9_best_model']
...
class Deep_xCas9(object):
    def __init__(self, filter_size, filter_num, node_1 = 80, node_2 = 60, l_rate = 0.005):
        ...
        self.features        = tf.placeholder(tf.float32, [None, 1])
        ...
        with tf.variable_scope('Fully_Connected_Layer_Feat'):
            W_fcl3       = tf.get_variable("W_fcl3", shape=[1, node_2])
            B_fcl3       = tf.get_variable("B_fcl3", shape=[node_2])
            L_fcl3_pre   = tf.nn.bias_add(tf.matmul(self.features, W_fcl3), B_fcl3)
            L_matmul_pre = tf.multiply(L_fcl2_pre, L_fcl3_pre)
            L_matmul = tf.nn.relu(L_matmul_pre)
            L_matmul_drop  = tf.layers.dropout(L_matmul, 0.3, self.is_training)
            
        with tf.variable_scope('Output_Layer'):
            W_out        = tf.get_variable("W_out", shape=[node_2, 1])
            B_out        = tf.get_variable("B_out", shape=[1])
            self.outputs = tf.nn.bias_add(tf.matmul(L_matmul_drop, W_out), B_out)
...
# Test Model
def Model_Inference(sess, TEST_X, TEST_Feature, model, args, load_episode, test_data_num, testvalbook, testvalsheet, col_index=1):
    test_batch = 500
    test_spearman = 0.0
    ...
    for i in range(int(ceil(float(TEST_X.shape[0])/float(test_batch)))):
        Dict = {model.inputs: TEST_X[i*test_batch:(i+1)*test_batch], model.is_training: False, model.features: TEST_Feature[i*test_batch:(i+1)*test_batch]}
    ...
    for test_value in (TEST_Z):
        testvalsheet[0].write(testval_row, testval_col, test_value[0])
        testval_row += 1
...
def getfile_inference(filenum):
    ...
    for l in range(1, data_n+1):
        ...
        if len(data_split) < 2: pass
        else:
            seq.append(data_split[0])
            features.append([data_split[1]])
    #loop end: l
    FILE.close()
    processed_full_seq = preprocess_seq(seq)

    return processed_full_seq, seq, features
#def end: getseq
...
testbook = xlsxwriter.Workbook('outputs/Variant_Cas9.xlsx')

TEST_X = []
TEST_Feature = []
testsheet = []

TEST_NUM = TEST_NUM_SET[0]
tmp_X, pre_X, tmp_Feature = getfile_inference(TEST_NUM)
TEST_X.append(tmp_X)
TEST_Feature.append(tmp_Feature)
...
for index in range(len(best_model_list)):
    ...
    test_row = 0
    testsheet.append([testbook.add_worksheet('{}'.format(best_model_path_list[index][2:]))])

    for index_X in range(np.shape(pre_X)[0]):
        testsheet[index][0].write(test_row, 0, pre_X[index_X])
        testsheet[index][0].write(test_row, 1, tmp_Feature[index_X][0])
        test_row += 1
    ...
    with tf.Session(config=conf) as sess:
        ...
        Model_Inference(sess, TEST_X[0], TEST_Feature[0], model, args, load_episode, TEST_NUM_SET[0], testbook, testsheet[index])
testbook.close()
